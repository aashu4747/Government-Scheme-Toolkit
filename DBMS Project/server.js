...
// Add another route to serve files from the 'public/main' directory
...
const db = mysql.createConnection({
  host: 'localhost',
  user: 'root',
  password: 'aashu123', // Replace 'your_password' with your MySQL password
  database: 'dbms'
});
          ...
          if (err) {
            ...
            if (isMatch) {
              ...
              if (user.email === 'resetschemedbms@gmail.com') {
                res.redirect('/main/admin_dashboard.html'); // Redirect to admin dashboard
              } else {
                res.redirect('/main/dashboard.html'); // Redirect to regular user dashboard
              }
            } else {
    ...
    if (err) {
      ...
      db.query('INSERT INTO users (username, email, password) VALUES (?, ?, ?)', [username, email, hashedPassword], (error, results) => {
        if (error) {
          ...
          res.redirect('main/dashboard.js');
        }
      });
    }
  });
...
function generateTemporaryPassword() {
  // Generate a random temporary password (you can customize the length and characters as needed)
...
app.post('/add-scheme', (req, res) => {
  ...
  const insertSchemeNameSQL = `
    INSERT INTO scheme_name (scheme_id, scheme_name)
    VALUES (?, ?)
  `;
